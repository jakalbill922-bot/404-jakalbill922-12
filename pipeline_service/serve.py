...
async def generate(prompt_image_file: UploadFile = File(...), seed: int = Form(-1)) -> StreamingResponse:
    ...
    try:
        ...
        # Return as streaming binary response with Content-Length
...
async def generate_spz(prompt_image_file: UploadFile = File(...), seed: int = Form(-1)) -> StreamingResponse:
    ...
    try:
        ...
        # Create buffer and get size
        spz_buffer = BytesIO(ply_compressed_bytes)
        buffer_size = len(ply_compressed_bytes)
        spz_buffer.seek(0)
        
        async def generate_chunks() -> AsyncGenerator[bytes, None]:
            chunk_size = 1024 * 1024  # 1 MB
            while chunk := spz_buffer.read(chunk_size):
                yield chunk
